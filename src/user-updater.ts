...
export class UserUpdater {
 ...
 this.userMdPath = path.join(workspacePath, 'USER.md');
 }
 ...
 */
 private updatePeople(facts: Fact[]): boolean {
 let content = fs.readFileSync(this.userMdPath, 'utf-8');
 let changed = false;
 ...
 const name = (fact.details as any)?.name || fact.content.split(' ')[0];
 ...
 if (changed) fs.writeFileSync(this.userMdPath, content);
 return changed;
 }
 ...
 * Creates the section if it doesn't exist
 */
 private updatePreferences(facts: Fact[]): boolean {
 let content = fs.readFileSync(this.userMdPath, 'utf-8');
 let changed = false;
 const date = new Date().toISOString().split('T')[0];
 ...
 if (content.includes(fact.content.substring(0, 40))) continue;
 ...
 */
 private updateProjects(facts: Fact[]): boolean {
 let content = fs.readFileSync(this.userMdPath, 'utf-8');
 let changed = false;
 const date = new Date().toISOString().split('T')[0];
 ...
 */
 private updateRecentlyLearned(facts: Fact[], type: string): boolean {
 let content = fs.readFileSync(this.userMdPath, 'utf-8');
 let changed = false;
 const date = new Date().toISOString().split('T')[0];
